...
class ExifService:
    ...
    def extract_exif(img_addr):
        try:            
            #exif dictionary creation since exif extraction makes each filed correspond to a nuimber
            #so we change it to relate each field by a field name
    ...
    #EXIF extraction and insertion in the DB indexed by object key
    @staticmethod
    def extract_and_insert_EXIF(directory, img):
        ...
        if not exif_data: 
            ...
            status, err = ExifService.dao.create(exif_data, img)            
            return status, err    
